...
* 在宿主機中新資料夾，該目錄用於映射**容器內**目錄`/config/.mdc`
  ![](/images/docker/8.jpg)
* 預設情況下，雲端配置為以**移動**方式整理文件，如需使用**連結**方式整理文件，請在[用戶面板](https://user.mvdc.top)中修改儲存
#### 如果要 移動 方式整理文件

<details>

* 左側為宿主機目錄（自行設定），右側為容器中的目錄（不可變）
* * **請勿修改`mdc.ini`檔案中`[common]`下`folder`相關配置**，這是容器內的目錄配置，修改會導致`來源資料夾找不到`
...
* 設定左側與右側的目錄為一致，至少設定兩個卷，一個卷為刮削目錄，另一個卷為輸出目錄；然後在下文生成後的`mdc.ini`文件中修改目錄配置
* 若使用硬鏈接，請確保刮削目錄與輸出目錄都處於**同一硬碟分區**下，否則會變成複製
* `/subs`字幕目錄可選
