...
![](/images/docker/8.jpg)
* 默认情况下，云配置为以**移动**方式整理文件，如需使用**链接**方式整理文件，请在[用户面板](https://user.mvdc.top)中修改保存
#### 如果要 移动 方式整理文件

<details>

* 左侧为宿主机目录（自行设置），右侧为容器中的目录（不可变）
* * **请勿修改`mdc.ini`文件中`[common]`下`folder`相关配置**，这是容器内的目录配置，修改会导致`源文件夹未找到`
...
* 设置左侧与右侧的目录为一致，至少设置两个卷，一个卷为刮削目录，另一个卷为输出目录；然后在下文生成后的`mdc.ini`文件中修改目录配置
* 若使用硬链接，请确保刮削目录与输出目录都处于**同一硬盘分区**下，否则会变成复制
* `/subs`字幕目录可选
