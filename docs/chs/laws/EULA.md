...
## 第一条：定义与双方责任
### 1.1 定义
* 软件：指 "Movie Data Crawler" 的任何平台软件、网站和以及与之相关的所有文档
* 经营者：Yohane Network Technology Limited
...
### 1.2 经营者责任
经营者确保在不影响网站稳定运行的前提下，努力完善程序的功能和稳定性，包括但不限于更新程序等手段。  
经营者不保证互联网中数据的完整性，不对用户操作所造成的数据丢失负责。
...
### 1.4 用户使用建议
为了确保阁下的体验，软件运行过程会随着需处理文件数量使用相应的时间，经营者提供以下使用建议：

* 定期备份数据： 强烈建议阁下定期备份使用软件期间生成的重要数据，以防止意外数据丢失或损坏的情况发生。
* 遵守法律法规： 阁下在使用软件时务必严格遵守所有适用的法律法规，包括但不限于著作权法和隐私法规，以确保合法合规的使用行为。
* 保持软件更新： 为了确保软件的最佳性能和安全性，阁下应随时安装并使用经营者提供的软件更新和修复程序。及时更新有助于弥补潜在漏洞并提供更好的使用体验。
...
## 第三条：许可授权
### 3.1 授权范围
经营者特此授权用户一个个人、不可转让的许可权，以便用户能够在遵循EULA条款的前提下，安装和使用软件。

### 3.2 个人用途
本软件许可仅限于个人用途。用户不得将软件用于商业或者盈利目的，除非经过经营者明确授权。

### 3.3 不可转让
用户不得出售、转让、出租、合租或以任何其他方式分发软件或其许可权（除经营者特别说明与优惠活动外）。  
...
### 3.5 许可期限
许可期限为用户订阅的过期时间。然而，经营者保留在用户违反EULA在内条款的情况下，终止或暂停该许可的权利。
...
* **安全保障**：经营者将定期更新软件以提供最新的安全性保障，确保阁下的数据和隐私得到保护。
* **满意保证**：经营者致力于提供最佳的使用体验。 

此外，如果阁下有**特殊的需求或需要个性化的支持**，经营者鼓励阁下与经营者联系。经营者提供一对一的远程支持和服务，以确保阁下的特殊需求得到满足，并为阁下提供最佳的使用体验。请注意，由于这些服务可能需要额外的资源和专业知识，因此会涉及到**额外的费用**。经营者将为阁下提供最高水平的支持和满意度！

### 5.2 定价和价格变动
经营者保留在任何时间更改软件订阅价格的权利。任何价格变动仅适用于新的订阅周期。

### 5.3 使用时长和定价权
经营者具有对不同使用时长（例如：月度、季度、年度、永久）的软件订阅进行定价的权利。
...
## 第六条：知识产权
### 6.1 知识产权归属
除软件开发所基于开源项目（例如CreativeDesignsGuru和Vuepress）等开源项目所衍生的专有代码和特殊说明外，软件及其所有组成部分（包括但不限于源代码、图标、界面设计、文档）的知识产权均归经营者所有。
基于开源项目衍生的专有代码的知识产权归经营者所有。  
...
### 6.3 数据准确性
经营者不对从互联网收集的数据的准确性、完整性或可靠性负责。但经营者会通过更新等方式尽力确保元数据的准确性和完整性。用户在使用软件时应自行验证这些数据的准确性。

### 6.4 不构成侵权
经营者明确声明，软件不含有任何侵犯第三方知识产权的内容。  
如有侵权行为与软件有关，请联系经营者，经营者将采取相应措施。

### 6.5 知识产权保护
用户不得以任何形式侵犯经营者或任何第三方的知识产权，包括但不限于通过非法复制、分发软件或元数据。
...
软件仅限于个人用途，未经经营者明确书面授权，不得用于商业或盈利目的。违反此规定者将被要求支付额外费用，并可能面临法律责任。
...
## 第八条：终止
### 8.1 许可终止条件
违反本协议的任何条款和条件将导致软件使用许可立即终止。经营者有权在不提前通知的情况下终止许可。

### 8.2 经营者权利
在确定用户有违反本协议的行为后，经营者有权禁用该用户的软件访问权限，并采取其他适当的法律和技术手段。

### 8.3 无退款政策
在许可终止的情况下，已支付的订阅费用将不予退还。如用户不想再继续使用软件，用户可在订阅期过后自行终止订阅。

### 8.4 数据处理
在许可终止后，经营者有权删除或保留与用户相关的所有数据和信息，但需遵循相关法律和数据保护政策。

### 8.5 不可抗力
* `不可抗力`是指用户与经营者双方不能合理控制、不可预见或即使预见亦无法避免的事件，该事件妨碍、影响或延误任何一方根据协议履行其全部或部分义务
* 该事件包括但不限于政府行为、自然灾害、战争、黑客攻击或任何其它类似事件
* 由于以上所述不可抗力事件致使协议的部分或全部不能履行或延迟履行，则用户与经营者双方于彼此间不承担任何违约责任

## 第九条：社区插件

### 9.1 社区插件概述
社区插件是由软件的社区用户开发的额外功能或扩展，可以通过包括但不限于在经营者网站和软件中下载、安装或启用。  
社区插件的开发者可自行设立最终用户许可协议或其他协议或其他条款，但不能与该协议在内的条款，隐私策略、免责声明相抵触。  
**经营者对社区中用户所开发的插件持中立态度，如果该插件侵犯了本EULA在内的条款，经营者可能会把该社区插件禁用并列入黑名单中，并保留诉讼的权力。**
