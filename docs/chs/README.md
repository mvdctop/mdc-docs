# 首页
* Movie Data Capture (MDC) 是一款强大的软件工具
...
## 1. 注册账号
* 在网页端的[用户面板](https://user.mvdc.top)注册账号并登录
...
* [中國大陸外支付](/cht/README.md#_2-购买激活码激活订阅)
### 当前内测阶段价格
🟢 **永久** 399`CNY` 56`USDT` （原价499，内测阶段限时优惠；存量用户转永久价格为减去之前的订阅费用）  
🟢 **一年** 99 `CNY` 14`USDT`
🟢 **半年** 55 `CNY`

* 内测阶段人工处理，**直接发送付款截图**至邮箱`yoshiko2.dev@gmail.com`，邮件标题：`付款截图`  
  永久只需发送附加用户名无需手动激活

* 将尽快给阁下发送激活信息，最快秒回最晚12小时内

![](/images/pay.png)


## 3. [下载](https://dl.mvdc.top)和运行
...
## 6. 检查文件命名，可参考[文件命名规则](/chs/naming.html)

* **根据影片类型在`本地配置`页右下选择相应`刮削类型`**
...
* 处理失败的影片会被记录，在程序所在目录下生成`failed_list.txt`
